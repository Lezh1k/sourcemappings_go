...
start = time.time()
p_parts_count = source_map.count(b";")
pnodes = [pnode] * (p_parts_count)
prev_node = pnode()
ss = 0
rix = 0
while True:
    se = source_map.find(b";", ss)
    if se == -1: 
        break
    sub = source_map[ss:se] if se != -1 else source_map[ss:]
    ss = se+1
    if not sub:
        pnodes[rix] = prev_node
        rix += 1
        continue
    
    sub_parts = sub.split(b":")    
    prev_node.s = sub_parts[0] if (len(sub_parts) >= 1 and sub_parts[0]) else prev_node.s 
    prev_node.l = sub_parts[1] if (len(sub_parts) >= 2 and sub_parts[1]) else prev_node.l
    prev_node.f = sub_parts[2] if (len(sub_parts) >= 3 and sub_parts[2]) else prev_node.f
    prev_node.j = sub_parts[3] if (len(sub_parts) >= 4 and sub_parts[3]) else prev_node.j
    prev_node.m = sub_parts[4] if (len(sub_parts) >= 5 and sub_parts[4]) else prev_node.m
    pnodes[rix] = prev_node
    rix += 1
end = time.time()

print('python took: {}'.format(end-start))
print('last_node: {}, nodes_count: {}'.format(pnodes[len(pnodes)-1], len(pnodes)))