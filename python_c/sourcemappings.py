...
class cnode_t(ctypes.Structure):
    ...
    def __str__(self):
        s = self.s[:self.s_] if self.s else ''
        l = self.l[:self.l_] if self.l else ''
        f = self.f[:self.f_] if self.f else ''
        j = self.j[:self.j_] if self.j else ''
        m = self.m[:self.m_] if self.m else ''
...
count = ctypes.c_int()
res = cf_decompile(b'1:2:1;:9;2:3:2;;;:5', ctypes.byref(count))
